...
# quadrotor_2 = Vehicle(mass_LQ, inertiaMatrix_LQ, armLength_1, omegaSqrToDragTorque, stdDevTorqueDisturbance)
...
class DroneDock_Env(gym.Env):
    ...
    def __init__(self,
        ...
        self.cur_pwm_cmd_2 = np.zeros(4)
        self.last_pwm_cmd_2 = np.zeros(4)
        self.thrustNormDes_1 = Vec3(0, 0, 0)
        self.thrustNormDes_2 = Vec3(0, 0, 0)
        self.angAccDes_1 = Vec3(0, 0, 0)
        self.angAccDes_2 = Vec3(0, 0, 0)        
        self.posControl_1 = PositionController(posCtrlNatFreq_1, posCtrlDampingRatio_1)
        # self.posControl_2 = PositionController(posCtrlNatFreq_1, posCtrlDampingRatio_1)
        self.posControl_2 = PositionController(posCtrlNatFreq_2, posCtrlDampingRatio_2)
        ...
        # self.attController_2 = QuadAttiControllerNested(att_timeConst_xy_1, att_timeConst_z_1, angVel_timeConst_xy_1, angVel_timeConst_z_1)
        # self.attController_2 = QuadAttiControllerNested(att_timeConst_xy_2, att_timeConst_z_2, angVel_timeConst_xy_2, angVel_timeConst_z_2)
    ...
    def step(self,
        ...
        accDes_2 = self.posControl_2.get_acceleration_command(Vec3(self.des_pos2), self.quadrotor_2._pos, self.quadrotor_2._vel)
        ...
        # self.angAccDes_2 = self.attController_2.get_angular_acceleration(self.thrustNormDes_2, self.quadrotor_2._att, self.quadrotor_2._omega)
        
        self.angAccDes_1 = self.attController_1.get_angular_acceleration(self.quadrotor_1._att, self.quadrotor_1._omega)
        self.angAccDes_2 = self.attController_2.get_angular_acceleration(self.quadrotor_2._att, self.quadrotor_2._omega)
        ...
        # pwm_cmd_1 = np.clip(pwm_cmd_1, 0, self.max_pwm)
        pwm_cmd_2 = self.mixer_2.get_motor_force_cmd(self.thrustNormDes_2, self.angAccDes_2)
        
        self.cur_pwm_cmd_1 = pwm_cmd_1
        self.cur_pwm_cmd_2 = pwm_cmd_2
        
        '''
        dwForce4Quad_1 = Vec3(0, 0, 0)
        if (self.quadrotor_2._pos.z > self.quadrotor_1._pos.z) and \
            (math.sqrt((self.quadrotor_2._pos.x+self.quadrotor_1._pos.x)**2+(self.quadrotor_2._pos.y+self.quadrotor_1._pos.y)**2) < (self.quadrotor_1._armlength + self.quadrotor_2._armlength)):
            dwForce4Quad_1 = downwash(self.quadrotor_2._pos, self.quadrotor_1._pos, self.quadrotor_2.get_motor_forces())
        '''
        
        self.quadrotor_1.run(self.dt, pwm_cmd_1)
        self.quadrotor_2.run(self.dt, pwm_cmd_2)
        

        obs_1, obs_2 = self.computeObs()
        reward_1, reward_2 = self.computeReward()
        
        self.last_pwm_cmd_1 = pwm_cmd_1
        self.last_pwm_cmd_2 = pwm_cmd_2
        ...
        return obs_1, obs_2, reward_1, reward_2, done, info
    ...
    #===================================================================================
    def convert_vehState(self, veh_state, des_pos, other_state=None):
        ...
        if self.highlevel_on:
            obs = np.concatenate((  veh_state.pos - other_state.pos,
                                    rotation_matrix, # 9
                                    veh_state.omega, # 3
    ...
    def computeObs(self):
        ...
        self.cur_state_2.cmd_bodyrates = self.angAccDes_2.to_array().flatten()
        
        obs_1 = self.convert_vehState(self.cur_state_1, self.des_pos1, other_state=self.cur_state_2)
        obs_2 = None
        # obs_2 = self.convert_vehState(self.cur_state_2, self.des_pos2)
    ...
    def computeReward(self, survival_reward=0.02, collision_penalty=-10):
        reward_1, reward_2 = 0, 0
        reward_1 += survival_reward \
            - 0.1 * np.linalg.norm(self.des_pos1 - self.cur_state_1.pos)\
            - 0.1 * np.linalg.norm(np.array([0, 0, 0]) - self.cur_state_1.ypr)\
            - 0.01 * np.linalg.norm((self.cur_pwm_cmd_1 - self.last_pwm_cmd_1), ord=1)
        '''
        reward_2 += survival_reward \
            - 0.1 * np.linalg.norm(self.des_pos2 - self.cur_state_2.pos)\
            - 0.05 * np.linalg.norm(np.array([0, 0, 0]) - self.cur_state_1.ypr)\
            - 0.01 * np.linalg.norm(np.array([0, 0, 0]) - self.cur_state_1.vel)
        '''
            
        if self.collision:
            reward_1 += collision_penalty
            reward_2 += collision_penalty
    ...
    def getMixer(self):
        ...
        # self.mixer_2 = QuadcopterMixer(self.quadrotor_1._mass, self.quadrotor_1._inertia, self.quadrotor_1._armlength/(2**0.5), self.quadrotor_1.speedSqrToTorque/self.quadrotor_1.speedSqrToThrust)
